...
perform_analysis_avg <- function(data, window_size) {
  ...
  for (i in 1:(nrow(data) - window_size)) {
    ...
    # Calculate historical average return for the current window
    historical_avg <- mean(current_window$return)
    
    # Store the forecast in the list
    forecast_results <- rbind(forecast_results, historical_avg)
...
# Function to perform analysis for different subsets using historical average
perform_analysis_subset_avg <- function(data, window_size) {
  # Initialize a list to store results
  subset_results <- list()
  
  # Perform analysis for usa[,2:5]
  fin_only <- perform_analysis_avg(data[, c(1, 2:5)], window_size)
  subset_results$fin_only <- fin_only
  
  # Perform analysis for usa[,2,6:8]
  macro_only <- perform_analysis_avg(data[, c(1, 2, 6:8)], window_size)
...
subset1_stats <- perform_analysis_avg(subset1, 60)
subset1_stats_plot <- subset1_stats[9]
subset1_stats <- subset1_stats[-9]
subset1_stats
subset2_stats <- perform_analysis_avg(subset2, 60)
subset2_stats_plot <- subset2_stats[9]
subset2_stats <- subset2_stats[-9]
subset2_stats
subset3_stats <- perform_analysis_avg(subset3, 60)
...
subset_results <- perform_analysis_subset_avg(usa, 60)
...
subset_results_before2010 <- perform_analysis_subset_avg(subset1, 60)
...
subset_results_after2010 <- perform_analysis_subset_avg(subset2, 60)
